	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (rs.next()) {
				...
				session.setAttribute("date", java.sql.Date.valueOf(LocalDate.now()));
				...
				HttpSession session  =req.getSession();
				session.setAttribute("client_login_error",1);
				
				RequestDispatcher d = req.getRequestDispatcher("/Client_Pages/log_in_client.jsp");
				d.forward(req, resp);
